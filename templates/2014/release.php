<article class="albumRating">
	<header class="releaseTitleBox">
		<h3 class="releaseTitleText"><?php echo $this->title; ?></h3>
		<p class="releaseDateText"><?php echo date('Y, M j', strtotime($this->releaseDate)); ?></p>
	</header>
		...
		<ol class="floatLeft releaseTracklist">
			<?php foreach($this->tracklist as $track) { ?>
				...
				<?php if(!empty($track['trackTabFile'])) { ?>
					<br /><span class="siteBlue">&raquo;</span> 
					<a class="releaseLink" href="contents/tabs/<?php echo $track['trackTabFile']; ?>">Guitar Pro tab</a></li>
				<?php } // End if ?>
			<?php } // End foreach ?>
		</ol>
		<div class="clear"></div>
